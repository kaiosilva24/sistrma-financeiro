...
const insertAuthRow = (method, active, extraData = null) => {
    getAccountDb().mutate('INSERT INTO auth (method, display_name, extra_data, active) VALUES (?, ?, ?, ?)', [method, method, extraData, active]);
};
const clearAuth = () => {
...
describe('/login', () => {
    ...
    it('should allow password login when OIDC is the active method', async () => {
        ...
        insertAuthRow('openid', 1);
        getAccountDb().mutate("UPDATE auth SET active = 0 WHERE method = 'password'");
        const res = await request(app)
            .post('/login')
...
describe('/server-prefs', () => {
    describe('POST /server-prefs', () => {
        ...
        it('should update existing server preferences', async () => {
            ...
            getAccountDb().mutate('INSERT INTO server_prefs (key, value) VALUES (?, ?)', ['flags.plugins', 'false']);
            // Update preferences
