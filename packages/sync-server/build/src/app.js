...
import * as emailAuthApp from './app-email-auth.js';
...
app.get('/admin-panel', (req, res) => res.sendFile(path.join(__dirname_app, 'static', 'admin.html')));
app.get('/user-login', (req, res) => res.sendFile(path.join(__dirname_app, 'static', 'login.html')));
...
app.use((req, res, next) => {
    ...
    res.set('Content-Security-Policy', "default-src 'self' blob:; img-src 'self' blob: data:; script-src 'self' 'unsafe-eval' blob:; style-src 'self' 'unsafe-inline'; font-src 'self' data:; connect-src http: https:;");
    next();
});
if (process.env.NODE_ENV === 'development') {
    console.log('Running in development mode - Proxying frontend routes to React Dev Server');
