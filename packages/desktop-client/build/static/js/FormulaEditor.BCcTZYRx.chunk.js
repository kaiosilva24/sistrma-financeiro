import { l as __toESM, n as require_jsx_runtime, r as require_react, t as theme$1 } from "./theme.LZxTPVqa.chunk.js";
import { b as styles, c as useTheme, ci as t$1, n as _objectWithoutPropertiesLoose, oi as Trans, t as _extends } from "./extends.opUJhDAp.chunk.js";
import { t as require_client } from "./client.COzYbjf1.chunk.js";
...
//#endregion
//#region ../../node_modules/@codemirror/commands/node_modules/@codemirror/language/dist/index.js
var _a$4;
/**
Node prop stored in a parser's top syntax node to provide the
facet that stores language-specific data for that language.
*/
var languageDataProp$4 = /* @__PURE__ */ new NodeProp();
/**
Syntax node prop used to register sublanguages. Should be added to
the top level node type for the language.
*/
var sublanguageProp$4 = /* @__PURE__ */ new NodeProp();
/**
	...
	*/
	constructor(data, parser, extraExtensions = [], name = "") {
		...
		if (!EditorState.prototype.hasOwnProperty("tree")) Object.defineProperty(EditorState.prototype, "tree", { get() {
			return syntaxTree$4(this);
		} });
		this.parser = parser;
		this.extension = [language$4.of(this), EditorState.languageData.of((state, pos, side) => {
...
var currentContext$4 = null;
/**
A parse context provided to parsers working on the editor content.
*/
var ParseContext$4 = class ParseContext$4 {
	...
	*/
	static create(parser, state, viewport) {
		return new ParseContext$4(parser, state, [], Tree.empty, 0, viewport, [], null);
	}
	startParse() {
	...
	*/
	...
	withoutTempSkipped(fragments) {
		for (let r; r = this.tempSkipped.pop();) fragments = cutFragments$4(fragments, r.from, r.to);
		return fragments;
	}
	/**
	@internal
	*/
	changes(changes, newState) {
		...
		return new ParseContext$4(this.parser, newState, fragments, tree, treeLen, viewport, skipped, this.scheduleOn);
	}
	/**
	@internal
	*/
...
var LanguageState$4 = class LanguageState$4 {
	constructor(context) {
		this.context = context;
	...
	apply(tr) {
		...
		return new LanguageState$4(newCx);
	}
	static init(state) {
		let vpTo = Math.min(3e3, state.doc.length);
		let parseState = ParseContext$4.create(state.facet(language$4).parser, state, {
			from: 0,
			to: vpTo
		});
		if (!parseState.work(20, vpTo)) parseState.takeTree();
		return new LanguageState$4(parseState);
	}
};
Language$4.state = /* @__PURE__ */ StateField.define({
	create: LanguageState$4.init,
	update(value, tr) {
		for (let e of tr.effects) if (e.is(Language$4.setState)) return e.value;
		if (tr.startState.facet(language$4) != tr.state.facet(language$4)) return LanguageState$4.init(tr.state);
		return value.apply(tr);
	}
});
var requestIdle$4 = (callback) => {
	let timeout = setTimeout(() => callback(), 500);
	return () => clearTimeout(timeout);
};
if (typeof requestIdleCallback != "undefined") requestIdle$4 = (callback) => {
	let idle = -1, timeout = setTimeout(() => {
		idle = requestIdleCallback(callback, { timeout: 400 });
	}, 100);
	return () => idle < 0 ? clearTimeout(timeout) : cancelIdleCallback(idle);
};
var isInputPending$4 = typeof navigator != "undefined" && ((_a$4 = navigator.scheduling) === null || _a$4 === void 0 ? void 0 : _a$4.isInputPending) ? () => navigator.scheduling.isInputPending() : null;
var parseWorker$4 = /* @__PURE__ */ ViewPlugin.fromClass(class ParseWorker {
	constructor(view) {
		this.view = view;
		this.working = null;
	...
	scheduleWork() {
		...
		if (field.tree != field.context.tree || !field.context.isDone(state.doc.length)) this.working = requestIdle$4(this.work);
	}
	work(deadline) {
		this.working = null;
		let now = Date.now();
		...
		let endTime = Date.now() + Math.min(this.chunkBudget, 100, deadline && !isInputPending$4 ? Math.max(25, deadline.timeRemaining() - 5) : 1e9);
...
*/
var language$4 = /* @__PURE__ */ Facet.define({
	combine(languages) {
...
function getIndentation$1(context, pos) {
	if (context instanceof EditorState) context = new IndentContext$1(context);
	for (let service of context.state.facet(indentService$1)) {
		let result = service(context, pos);
		if (result !== void 0) return result;
	}
	let tree = syntaxTree$4(context.state);
...
*/
var indentNodeProp$2 = /* @__PURE__ */ new NodeProp();
...
function indentFor$1(stack, cx, pos) {
	for (let cur = stack; cur; cur = cur.next) {
		...
		if (strategy) return strategy(TreeIndentContext$1.create(cx, pos, cur));
	}
	return 0;
}
...
function indentStrategy$1(tree) {
	...
	if (first && (close = first.type.prop(NodeProp.closedBy))) {
		...
		return (cx) => delimitedStrategy$1(cx, true, 1, void 0, closed && !ignoreClosed$1(cx) ? last.from : void 0);
	}
...
*/
var TreeIndentContext$1 = class TreeIndentContext$1 extends IndentContext$1 {
	...
	*/
	static create(base, pos, context) {
		return new TreeIndentContext$1(base, pos, context);
	}
	/**
	...
	*/
	baseIndentFor(node) {
		...
		for (;;) {
			...
			if (isParent$1(atBreak, node)) break;
...
var DefaultScanDist$1 = 1e4, DefaultBrackets$1 = "()[]{}";
/**
...
place.
*/
var bracketMatchingHandle$1 = /* @__PURE__ */ new NodeProp();
...
function matchBrackets$1(state, pos, dir, config = {}) {
	...
	let tree = syntaxTree$4(state), node = tree.resolveInner(pos, dir);
	for (let cur = node; cur; cur = cur.parent) {
		let matches = matchingNodes$1(cur.type, dir, brackets);
		if (matches && cur.from < cur.to) {
			let handle = findHandle$1(cur);
			if (handle && (dir > 0 ? pos >= handle.from && pos < handle.to : pos > handle.from && pos <= handle.to)) return matchMarkedBrackets$1(state, pos, dir, cur, handle, matches, brackets);
		}
	}
...
function matchMarkedBrackets$1(_state, _pos, dir, token, handle, matching, brackets) {
	...
	if (cursor && (dir < 0 ? cursor.childBefore(token.from) : cursor.childAfter(token.to))) do
		if (dir < 0 ? cursor.to <= token.from : cursor.from >= token.to) {
			...
			} else if (matchingNodes$1(cursor.type, dir, brackets)) depth++;
...
var noTokens$4 = /* @__PURE__ */ Object.create(null);
var typeArray$4 = [NodeType.none];
var warned$4 = [];
var byTag$4 = /* @__PURE__ */ Object.create(null);
var defaultTable$4 = /* @__PURE__ */ Object.create(null);
...
function warnForPart$4(part, msg) {
	if (warned$4.indexOf(part) > -1) return;
	warned$4.push(part);
	console.warn(msg);
}
function createTokenType$4(extra, tagStr) {
	...
	for (let name of tagStr.split(" ")) {
		...
		for (let part of name.split(".")) {
			...
			if (!value) warnForPart$4(part, `Unknown highlighting tag ${part}`);
			else if (typeof value == "function") if (!found.length) warnForPart$4(part, `Modifier ${part} used at start of tag`);
			else found = found.map(value);
			else if (found.length) warnForPart$4(part, `Tag ${part} used as modifier`);
...
function moveBySyntax(state, start, forward) {
	let pos = syntaxTree$4(state).resolveInner(start.head);
	...
	if (bracket && (match = forward ? matchBrackets$1(state, pos.from, 1) : matchBrackets$1(state, pos.to, -1)) && match.matched) newPos = forward ? match.end.to : match.end.from;
...
function toMatchingBracket(state, dispatch, extend) {
	let found = false, selection = updateSel(state.selection, (range) => {
		let matching = matchBrackets$1(state, range.head, -1) || matchBrackets$1(state, range.head, 1) || range.head > 0 && matchBrackets$1(state, range.head - 1, 1) || range.head < state.doc.length && matchBrackets$1(state, range.head + 1, -1);
...
var selectParentSyntax = ({ state, dispatch }) => {
	let selection = updateSel(state.selection, (range) => {
		let tree = syntaxTree$4(state), stack = tree.resolveStack(range.from, 1);
		if (range.empty) {
...
var deleteByChar = (target, forward, byIndentUnit) => deleteBy(target, (range) => {
	...
	if (byIndentUnit && !forward && pos > line.from && pos < line.from + 200 && !/[^ \t]/.test(before = line.text.slice(0, pos - line.from))) {
		...
		let drop = countColumn(before, state.tabSize) % getIndentUnit$2(state) || getIndentUnit$2(state);
...
function isBetweenBrackets(state, pos) {
	...
	let context = syntaxTree$4(state).resolveInner(pos);
...
function newlineAndIndent(atEof) {
	return ({ state, dispatch }) => {
		...
		let changes = state.changeByRange((range) => {
			...
			let cx = new IndentContext$1(state, {
				simulateBreak: from,
				simulateDoubleBreak: !!explode
			});
			let indent = getIndentation$1(cx, from);
			...
			let insert = ["", indentString$1(state, indent)];
			if (explode) insert.push(indentString$1(state, cx.lineIndent(line.from, -1)));
			return {
				changes: {
					from,
					to,
...
var indentSelection = ({ state, dispatch }) => {
	...
	let context = new IndentContext$1(state, { overrideIndentation: (start) => {
		let found = updated[start];
		return found == null ? -1 : found;
	} });
	let changes = changeBySelectedLine(state, (line, changes, range) => {
		let indent = getIndentation$1(context, line.from);
...
var indentMore = ({ state, dispatch }) => {
	...
	dispatch(state.update(changeBySelectedLine(state, (line, changes) => {
		changes.push({
			from: line.from,
			insert: state.facet(indentUnit$2)
		});
...
var indentLess = ({ state, dispatch }) => {
	...
	dispatch(state.update(changeBySelectedLine(state, (line, changes) => {
		...
		let insert = indentString$1(state, Math.max(0, col - getIndentUnit$2(state)));
...
var gotoLine = (view) => {
	...
	if (!panel) {
		...
		if (view.state.field(dialogField, false) == null) effects.push(StateEffect.appendConfig.of([dialogField, baseTheme$1$4]));
...
var baseTheme$1$4 = /* @__PURE__ */ EditorView.baseTheme({ ".cm-panel.cm-gotoLine": {
	padding: "2px 6px 4px",
	position: "relative",
	"& label": { fontSize: "80%" },
	"& [name=close]": {
...
//#endregion
//#region ../../node_modules/@codemirror/autocomplete/node_modules/@codemirror/language/dist/index.js
var _a$3;
/**
Node prop stored in a parser's top syntax node to provide the
facet that stores language-specific data for that language.
*/
var languageDataProp$3 = /* @__PURE__ */ new NodeProp();
/**
Syntax node prop used to register sublanguages. Should be added to
the top level node type for the language.
*/
var sublanguageProp$3 = /* @__PURE__ */ new NodeProp();
/**
A language object manages parsing and per-language
[metadata](https://codemirror.net/6/docs/ref/#state.EditorState.languageDataAt). Parse data is
managed as a [Lezer](https://lezer.codemirror.net) tree. The class
...
var Language$3 = class {
	...
	*/
	constructor(data, parser, extraExtensions = [], name = "") {
		...
		this.name = name;
		if (!EditorState.prototype.hasOwnProperty("tree")) Object.defineProperty(EditorState.prototype, "tree", { get() {
			return syntaxTree$3(this);
		} });
		this.parser = parser;
		this.extension = [language$3.of(this), EditorState.languageData.of((state, pos, side) => {
...
var currentContext$3 = null;
/**
A parse context provided to parsers working on the editor content.
*/
var ParseContext$3 = class ParseContext$3 {
	constructor(parser, state, fragments = [], tree, treeLen, viewport, skipped, scheduleOn) {
		this.parser = parser;
		this.state = state;
		this.fragments = fragments;
	...
	*/
	static create(parser, state, viewport) {
		return new ParseContext$3(parser, state, [], Tree.empty, 0, viewport, [], null);
	}
	startParse() {
	...
	*/
	...
	withoutTempSkipped(fragments) {
		for (let r; r = this.tempSkipped.pop();) fragments = cutFragments$3(fragments, r.from, r.to);
		return fragments;
	}
	/**
	@internal
	*/
	changes(changes, newState) {
		...
		return new ParseContext$3(this.parser, newState, fragments, tree, treeLen, viewport, skipped, this.scheduleOn);
	}
	/**
	@internal
	*/
...
var LanguageState$3 = class LanguageState$3 {
	constructor(context) {
		this.context = context;
		this.tree = context.tree;
	}
	apply(tr) {
		...
		let upto = this.context.treeLen == tr.startState.doc.length ? void 0 : Math.max(tr.changes.mapPos(this.context.treeLen), newCx.viewport.to);
		if (!newCx.work(20, upto)) newCx.takeTree();
		return new LanguageState$3(newCx);
	}
	static init(state) {
		let vpTo = Math.min(3e3, state.doc.length);
		let parseState = ParseContext$3.create(state.facet(language$3).parser, state, {
			from: 0,
			to: vpTo
		});
		if (!parseState.work(20, vpTo)) parseState.takeTree();
		return new LanguageState$3(parseState);
	}
};
Language$3.state = /* @__PURE__ */ StateField.define({
	create: LanguageState$3.init,
	update(value, tr) {
		for (let e of tr.effects) if (e.is(Language$3.setState)) return e.value;
		if (tr.startState.facet(language$3) != tr.state.facet(language$3)) return LanguageState$3.init(tr.state);
		return value.apply(tr);
	}
});
var requestIdle$3 = (callback) => {
	let timeout = setTimeout(() => callback(), 500);
	return () => clearTimeout(timeout);
};
if (typeof requestIdleCallback != "undefined") requestIdle$3 = (callback) => {
	let idle = -1, timeout = setTimeout(() => {
		idle = requestIdleCallback(callback, { timeout: 400 });
	}, 100);
	return () => idle < 0 ? clearTimeout(timeout) : cancelIdleCallback(idle);
};
var isInputPending$3 = typeof navigator != "undefined" && ((_a$3 = navigator.scheduling) === null || _a$3 === void 0 ? void 0 : _a$3.isInputPending) ? () => navigator.scheduling.isInputPending() : null;
var parseWorker$3 = /* @__PURE__ */ ViewPlugin.fromClass(class ParseWorker {
	constructor(view) {
		this.view = view;
		this.working = null;
		this.workScheduled = 0;
	...
	scheduleWork() {
		...
		if (field.tree != field.context.tree || !field.context.isDone(state.doc.length)) this.working = requestIdle$3(this.work);
	}
	work(deadline) {
		this.working = null;
		let now = Date.now();
		...
		if (field.tree == field.context.tree && field.context.isDone(vpTo + 1e5)) return;
		let endTime = Date.now() + Math.min(this.chunkBudget, 100, deadline && !isInputPending$3 ? Math.max(25, deadline.timeRemaining() - 5) : 1e9);
		let viewportFirst = field.context.treeLen < vpTo && state.doc.length > vpTo + 1e3;
		let done = field.context.work(() => {
...
current language on a state.
*/
var language$3 = /* @__PURE__ */ Facet.define({
	combine(languages) {
		return languages.length ? languages[0] : null;
	},
	enables: (language) => [
...
tags.meta, tags.link, tags.heading, tags.emphasis, tags.strong, tags.strikethrough, tags.keyword, tags.atom, tags.bool, tags.url, tags.contentSeparator, tags.labelName, tags.literal, tags.inserted, tags.string, tags.deleted, tags.regexp, tags.escape, tags.string, tags.variableName, tags.variableName, tags.typeName, tags.namespace, tags.className, tags.variableName, tags.macroName, tags.propertyName, tags.comment, tags.invalid;
var noTokens$3 = /* @__PURE__ */ Object.create(null);
var typeArray$3 = [NodeType.none];
var warned$3 = [];
var byTag$3 = /* @__PURE__ */ Object.create(null);
var defaultTable$3 = /* @__PURE__ */ Object.create(null);
for (let [legacyName, name] of [
	["variable", "variableName"],
	["variable-2", "variableName.special"],
	["string-2", "string.special"],
...
function warnForPart$3(part, msg) {
	if (warned$3.indexOf(part) > -1) return;
	warned$3.push(part);
	console.warn(msg);
}
function createTokenType$3(extra, tagStr) {
	...
	for (let name of tagStr.split(" ")) {
		...
		for (let part of name.split(".")) {
			let value = extra[part] || tags[part];
			if (!value) warnForPart$3(part, `Unknown highlighting tag ${part}`);
			else if (typeof value == "function") if (!found.length) warnForPart$3(part, `Modifier ${part} used at start of tag`);
			else found = found.map(value);
			else if (found.length) warnForPart$3(part, `Tag ${part} used as modifier`);
			else found = Array.isArray(value) ? value : [value];
		}
		for (let tag of found) tags$1.push(tag);
	}
...
var CompletionContext = class {
	...
	*/
	tokenBefore(types) {
		let token = syntaxTree$3(this.state).resolveInner(this.pos, -1);
...
function nodeStart(state, pos) {
	let tree = syntaxTree$3(state).resolveInner(pos + 1);
	return tree.parent && tree.from == pos;
}
function probablyInString(state, pos, quoteToken, prefixes) {
	let node = syntaxTree$3(state).resolveInner(pos, -1);
...
//#endregion
//#region ../../node_modules/@uiw/codemirror-extensions-basic-setup/node_modules/@codemirror/language/dist/index.js
var _a$2;
/**
Node prop stored in a parser's top syntax node to provide the
facet that stores language-specific data for that language.
*/
var languageDataProp$2 = /* @__PURE__ */ new NodeProp();
/**
Syntax node prop used to register sublanguages. Should be added to
the top level node type for the language.
*/
var sublanguageProp$2 = /* @__PURE__ */ new NodeProp();
/**
A language object manages parsing and per-language
[metadata](https://codemirror.net/6/docs/ref/#state.EditorState.languageDataAt). Parse data is
managed as a [Lezer](https://lezer.codemirror.net) tree. The class
...
var Language$2 = class {
	...
	*/
	constructor(data, parser, extraExtensions = [], name = "") {
		...
		this.name = name;
		if (!EditorState.prototype.hasOwnProperty("tree")) Object.defineProperty(EditorState.prototype, "tree", { get() {
			return syntaxTree$2(this);
		} });
		this.parser = parser;
		this.extension = [language$2.of(this), EditorState.languageData.of((state, pos, side) => {
...
var currentContext$2 = null;
/**
A parse context provided to parsers working on the editor content.
*/
var ParseContext$2 = class ParseContext$2 {
	constructor(parser, state, fragments = [], tree, treeLen, viewport, skipped, scheduleOn) {
		this.parser = parser;
		this.state = state;
		this.fragments = fragments;
	...
	*/
	static create(parser, state, viewport) {
		return new ParseContext$2(parser, state, [], Tree.empty, 0, viewport, [], null);
	}
	startParse() {
	...
	*/
	...
	withoutTempSkipped(fragments) {
		for (let r; r = this.tempSkipped.pop();) fragments = cutFragments$2(fragments, r.from, r.to);
		return fragments;
	}
	/**
	@internal
	*/
	changes(changes, newState) {
		...
		return new ParseContext$2(this.parser, newState, fragments, tree, treeLen, viewport, skipped, this.scheduleOn);
	}
	/**
	@internal
	*/
...
var LanguageState$2 = class LanguageState$2 {
	constructor(context) {
		this.context = context;
		this.tree = context.tree;
	}
	apply(tr) {
		...
		let upto = this.context.treeLen == tr.startState.doc.length ? void 0 : Math.max(tr.changes.mapPos(this.context.treeLen), newCx.viewport.to);
		if (!newCx.work(20, upto)) newCx.takeTree();
		return new LanguageState$2(newCx);
	}
	static init(state) {
		let vpTo = Math.min(3e3, state.doc.length);
		let parseState = ParseContext$2.create(state.facet(language$2).parser, state, {
			from: 0,
			to: vpTo
		});
		if (!parseState.work(20, vpTo)) parseState.takeTree();
		return new LanguageState$2(parseState);
	}
};
Language$2.state = /* @__PURE__ */ StateField.define({
	create: LanguageState$2.init,
	update(value, tr) {
		for (let e of tr.effects) if (e.is(Language$2.setState)) return e.value;
		if (tr.startState.facet(language$2) != tr.state.facet(language$2)) return LanguageState$2.init(tr.state);
		return value.apply(tr);
	}
});
var requestIdle$2 = (callback) => {
	let timeout = setTimeout(() => callback(), 500);
	return () => clearTimeout(timeout);
};
if (typeof requestIdleCallback != "undefined") requestIdle$2 = (callback) => {
	let idle = -1, timeout = setTimeout(() => {
		idle = requestIdleCallback(callback, { timeout: 400 });
	}, 100);
	return () => idle < 0 ? clearTimeout(timeout) : cancelIdleCallback(idle);
};
var isInputPending$2 = typeof navigator != "undefined" && ((_a$2 = navigator.scheduling) === null || _a$2 === void 0 ? void 0 : _a$2.isInputPending) ? () => navigator.scheduling.isInputPending() : null;
var parseWorker$2 = /* @__PURE__ */ ViewPlugin.fromClass(class ParseWorker {
	constructor(view) {
		this.view = view;
		this.working = null;
		this.workScheduled = 0;
	...
	scheduleWork() {
		...
		if (field.tree != field.context.tree || !field.context.isDone(state.doc.length)) this.working = requestIdle$2(this.work);
	}
	work(deadline) {
		this.working = null;
		let now = Date.now();
		...
		if (field.tree == field.context.tree && field.context.isDone(vpTo + 1e5)) return;
		let endTime = Date.now() + Math.min(this.chunkBudget, 100, deadline && !isInputPending$2 ? Math.max(25, deadline.timeRemaining() - 5) : 1e9);
		let viewportFirst = field.context.treeLen < vpTo && state.doc.length > vpTo + 1e3;
		let done = field.context.work(() => {
...
current language on a state.
*/
var language$2 = /* @__PURE__ */ Facet.define({
	combine(languages) {
		return languages.length ? languages[0] : null;
	},
	enables: (language) => [
...
When not set, this defaults to 2 spaces.
*/
var indentUnit$1 = /* @__PURE__ */ Facet.define({ combine: (values) => {
	if (!values.length) return "  ";
	let unit = values[0];
	if (!unit || /\S/.test(unit) || Array.from(unit).some((e) => e != unit[0])) throw new Error("Invalid indent unit: " + JSON.stringify(values[0]));
	return unit;
...
indicates that no definitive indentation can be determined.
*/
var indentNodeProp$1 = /* @__PURE__ */ new NodeProp();
...
function foldGutter(config = {}) {
	...
	let markers = ViewPlugin.fromClass(class {
		...
		update(update) {
			if (update.docChanged || update.viewportChanged || update.startState.facet(language$2) != update.state.facet(language$2) || update.startState.field(foldState, false) != update.state.field(foldState, false) || syntaxTree$2(update.startState) != syntaxTree$2(update.state) || fullConfig.foldingChanged(update)) this.markers = this.buildMarkers(update.view);
		}
		buildMarkers(view) {
			let builder = new RangeSetBuilder();
...
*/
var HighlightStyle$2 = class HighlightStyle$2 {
	constructor(specs, options) {
		...
		this.scope = scopeOpt instanceof Language$2 ? (type) => type.prop(languageDataProp$2) == scopeOpt.data : scopeOpt ? (type) => type == scopeOpt : void 0;
	...
	*/
	static define(specs, options) {
		return new HighlightStyle$2(specs, options || {});
	}
};
var highlighterFacet$2 = /* @__PURE__ */ Facet.define();
var fallbackHighlighter$2 = /* @__PURE__ */ Facet.define({ combine(values) {
	return values.length ? [values[0]] : null;
} });
function getHighlighters$2(state) {
	let main = state.facet(highlighterFacet$2);
	return main.length ? main : state.facet(fallbackHighlighter$2);
}
/**
...
*/
function syntaxHighlighting$2(highlighter, options) {
	let ext = [treeHighlighter$2], themeType;
	if (highlighter instanceof HighlightStyle$2) {
		if (highlighter.module) ext.push(EditorView.styleModule.of(highlighter.module));
		themeType = highlighter.themeType;
	}
	if (options === null || options === void 0 ? void 0 : options.fallback) ext.push(fallbackHighlighter$2.of(highlighter));
	else if (themeType) ext.push(highlighterFacet$2.computeN([EditorView.darkTheme], (state) => {
		return state.facet(EditorView.darkTheme) == (themeType == "dark") ? [highlighter] : [];
	}));
	else ext.push(highlighterFacet$2.of(highlighter));
	return ext;
}
var TreeHighlighter$2 = class {
	constructor(view) {
		this.markCache = Object.create(null);
		this.tree = syntaxTree$2(view.state);
		this.decorations = this.buildDeco(view, getHighlighters$2(view.state));
		this.decoratedTo = view.viewport.to;
	}
	update(update) {
		let tree = syntaxTree$2(update.state), highlighters = getHighlighters$2(update.state);
		let styleChange = highlighters != getHighlighters$2(update.startState);
...
var treeHighlighter$2 = /* @__PURE__ */ Prec.high(/* @__PURE__ */ ViewPlugin.fromClass(TreeHighlighter$2, { decorations: (v) => v.decorations }));
/**
A default highlight style (works well with light themes).
*/
var defaultHighlightStyle$2 = /* @__PURE__ */ HighlightStyle$2.define([
	{
		tag: tags.meta,
		color: "#404740"
	},
...
var noTokens$2 = /* @__PURE__ */ Object.create(null);
var typeArray$2 = [NodeType.none];
var warned$2 = [];
var byTag$2 = /* @__PURE__ */ Object.create(null);
var defaultTable$2 = /* @__PURE__ */ Object.create(null);
for (let [legacyName, name] of [
	["variable", "variableName"],
	["variable-2", "variableName.special"],
	["string-2", "string.special"],
...
function warnForPart$2(part, msg) {
	if (warned$2.indexOf(part) > -1) return;
	warned$2.push(part);
	console.warn(msg);
}
function createTokenType$2(extra, tagStr) {
	...
	for (let name of tagStr.split(" ")) {
		...
		for (let part of name.split(".")) {
			let value = extra[part] || tags[part];
			if (!value) warnForPart$2(part, `Unknown highlighting tag ${part}`);
			else if (typeof value == "function") if (!found.length) warnForPart$2(part, `Modifier ${part} used at start of tag`);
			else found = found.map(value);
			else if (found.length) warnForPart$2(part, `Tag ${part} used as modifier`);
			else found = Array.isArray(value) ? value : [value];
		}
		for (let tag of found) tags$1.push(tag);
	}
...
var basicSetup = function basicSetup(options) {
	...
	if (options.syntaxHighlighting !== false) extensions.push(syntaxHighlighting$2(defaultHighlightStyle$2, { fallback: true }));
	...
	if (options.tabSize && typeof options.tabSize === "number") extensions.push(indentUnit$1.of(" ".repeat(options.tabSize)));
	return extensions.concat([keymap.of(keymaps.flat())]).filter(Boolean);
};
//#endregion
//#region ../../node_modules/@codemirror/theme-one-dark/node_modules/@codemirror/language/dist/index.js
var _a$1;
/**
Node prop stored in a parser's top syntax node to provide the
facet that stores language-specific data for that language.
...
var ParseContext$1 = class ParseContext$1 {
	...
	*/
	static create(parser, state, viewport) {
		return new ParseContext$1(parser, state, [], Tree.empty, 0, viewport, [], null);
	}
	startParse() {
...
var parseWorker$1 = /* @__PURE__ */ ViewPlugin.fromClass(class ParseWorker {
	constructor(view) {
		this.view = view;
		this.working = null;
		this.workScheduled = 0;
...
[tags](https://lezer.codemirror.net/docs/ref#highlight.Tag).
*/
var HighlightStyle$1 = class HighlightStyle$1 {
	constructor(specs, options) {
		this.specs = specs;
		let modSpec;
		function def(spec) {
...
var highlighterFacet$1 = /* @__PURE__ */ Facet.define();
var fallbackHighlighter$1 = /* @__PURE__ */ Facet.define({ combine(values) {
	return values.length ? [values[0]] : null;
} });
...
function syntaxHighlighting$1(highlighter, options) {
	...
	if (options === null || options === void 0 ? void 0 : options.fallback) ext.push(fallbackHighlighter$1.of(highlighter));
	else if (themeType) ext.push(highlighterFacet$1.computeN([EditorView.darkTheme], (state) => {
		return state.facet(EditorView.darkTheme) == (themeType == "dark") ? [highlighter] : [];
	}));
	else ext.push(highlighterFacet$1.of(highlighter));
	return ext;
}
var TreeHighlighter$1 = class {
	constructor(view) {
		this.markCache = Object.create(null);
	...
	update(update) {
		let tree = syntaxTree$1(update.state), highlighters = getHighlighters$1(update.state);
...
var treeHighlighter$1 = /* @__PURE__ */ Prec.high(/* @__PURE__ */ ViewPlugin.fromClass(TreeHighlighter$1, { decorations: (v) => v.decorations }));
tags.meta, tags.link, tags.heading, tags.emphasis, tags.strong, tags.strikethrough, tags.keyword, tags.atom, tags.bool, tags.url, tags.contentSeparator, tags.labelName, tags.literal, tags.inserted, tags.string, tags.deleted, tags.regexp, tags.escape, tags.string, tags.variableName, tags.variableName, tags.typeName, tags.namespace, tags.className, tags.variableName, tags.macroName, tags.propertyName, tags.comment, tags.invalid;
...
}, { dark: true }), /* @__PURE__ */ syntaxHighlighting$1(/* @__PURE__ */ HighlightStyle$1.define([
	{
		tag: tags.keyword,
		color: violet
	},
...
//#endregion
//#region src/components/codemirror/autocompleteTabAccept.ts
var autocompleteTabAccept = keymap.of([{
	key: "Tab",
	run: (view) => {
...
* `indentWithTab`.
*/
var autocompleteTabAcceptHighest = Prec.highest(autocompleteTabAccept);
//#endregion
...
Direction.RTL, Direction.LTR;
//#endregion
//#region src/components/formula/queryModeFunctions.ts
var queryModeFunctions = {
	SUM: {
		name: "SUM",
		description: t$1("Returns the sum of all numbers in a range."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	},
	AVERAGE: {
		name: "AVERAGE",
		description: t$1("Returns the average of all numbers in a range."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	AVERAGEA: {
		name: "AVERAGEA",
		description: t$1("Returns the average, including text and logical values."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	},
	COUNT: {
		name: "COUNT",
		description: t$1("Counts the number of numeric values."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	COUNTA: {
		name: "COUNTA",
		description: t$1("Counts non-empty values."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	COUNTBLANK: {
		name: "COUNTBLANK",
		description: t$1("Counts empty cells."),
		parameters: [{
			name: "range",
			description: "Range"
		}]
	...
	COUNTIF: {
		name: "COUNTIF",
		description: t$1("Counts cells that meet a criteria."),
		parameters: [{
			name: "range",
			description: "Range"
		}, {
	...
	COUNTIFS: {
		name: "COUNTIFS",
		description: t$1("Counts cells that meet multiple criteria."),
		parameters: [{
			name: "range1",
			description: "Range1"
		}, {
	...
	MAX: {
		name: "MAX",
		description: t$1("Returns the maximum value."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	MAXA: {
		name: "MAXA",
		description: t$1("Returns the maximum value, including text and logical values."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	MIN: {
		name: "MIN",
		description: t$1("Returns the minimum value."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	MINA: {
		name: "MINA",
		description: t$1("Returns the minimum value, including text and logical values."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	ABS: {
		name: "ABS",
		description: t$1("Returns the absolute value of a number."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	ROUND: {
		name: "ROUND",
		description: t$1("Rounds a number to specified decimals."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	ROUNDDOWN: {
		name: "ROUNDDOWN",
		description: t$1("Rounds down to specified decimals."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	ROUNDUP: {
		name: "ROUNDUP",
		description: t$1("Rounds up to specified decimals."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	BUDGET_QUERY: {
		name: "BUDGET_QUERY",
		description: t$1("Evaluate a budget query using extracted parameters. Supply dimension, categories, and timeframe explicitly."),
		parameters: [
			{
				name: "dimension",
				description: "One of: budgeted, spent, balance_start, balance_end, goal (string)"
			},
			{
				name: "categories",
				description: "Categories result from QUERY_EXTRACT_CATEGORIES() (array)"
			},
			{
				name: "timeframe_start",
				description: "Start month from QUERY_EXTRACT_TIMEFRAME_START() (string)"
			},
			{
				name: "timeframe_end",
	...
	QUERY_EXTRACT_CATEGORIES: {
		...
		description: t$1("Extract category IDs from a named query."),
		parameters: [{
			name: "queryName",
			description: "Name of the saved query (string)"
		}]
	},
	QUERY_EXTRACT_TIMEFRAME_START: {
		name: "QUERY_EXTRACT_TIMEFRAME_START",
		description: t$1("Extract the start month from a named query timeframe."),
		parameters: [{
			name: "queryName",
			description: "Name of the saved query (string)"
		}]
	...
	QUERY_EXTRACT_TIMEFRAME_END: {
		...
		description: t$1("Extract the end month from a named query timeframe."),
		parameters: [{
			name: "queryName",
			description: "Name of the saved query (string)"
		}]
	...
	FLOOR: {
		name: "FLOOR",
		description: t$1("Rounds down to nearest multiple of significance."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	CEILING: {
		name: "CEILING",
		description: t$1("Rounds up to nearest multiple of significance."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	POWER: {
		name: "POWER",
		description: t$1("Returns base raised to the power of exponent."),
		parameters: [{
			name: "base",
			description: "Base"
		}, {
	...
	SQRT: {
		name: "SQRT",
		description: t$1("Returns the square root."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	MOD: {
		name: "MOD",
		description: t$1("Returns the remainder of division."),
		parameters: [{
			name: "dividend",
	...
	PI: {
		name: "PI",
		description: t$1("Returns the value of PI."),
		parameters: []
	},
	SIN: {
		name: "SIN",
		description: t$1("Returns the sine of an angle."),
		parameters: [{
			name: "angle",
			description: "Angle"
		}]
	...
	COS: {
		name: "COS",
		description: t$1("Returns the cosine of an angle."),
		parameters: [{
			name: "angle",
			description: "Angle"
		}]
	...
	TAN: {
		name: "TAN",
		description: t$1("Returns the tangent of an angle."),
		parameters: [{
			name: "angle",
			description: "Angle"
		}]
	...
	LN: {
		name: "LN",
		description: t$1("Returns the natural logarithm."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	LOG: {
		name: "LOG",
		description: t$1("Returns the logarithm to specified base."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	LOG10: {
		name: "LOG10",
		description: t$1("Returns the base-10 logarithm."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	EXP: {
		name: "EXP",
		description: t$1("Returns e raised to the power of number."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	PRODUCT: {
		name: "PRODUCT",
		description: t$1("Returns the product of all numbers."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	SUMIF: {
		name: "SUMIF",
		description: t$1("Sums cells that meet a criteria."),
		parameters: [
			{
				name: "range",
				description: "Range"
			...
			{
				name: "sum_range",
				description: "SumRange"
			}
		]
	},
	SUMIFS: {
		name: "SUMIFS",
		description: t$1("Sums cells that meet multiple criteria."),
		parameters: [
			{
				name: "sum_range",
				description: "SumRange"
	...
	SUMPRODUCT: {
		name: "SUMPRODUCT",
		description: t$1("Multiplies corresponding elements and returns the sum."),
		parameters: [{
			name: "array1",
			description: "Array1"
		}, {
	...
	SUMSQ: {
		name: "SUMSQ",
		description: t$1("Returns the sum of the squares."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	MEDIAN: {
		name: "MEDIAN",
		description: t$1("Returns the median value."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	MODE: {
		name: "MODE",
		description: t$1("Returns the most frequently occurring value."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	STDEV: {
		name: "STDEV",
		description: t$1("Returns the standard deviation of a sample."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	STDEVP: {
		name: "STDEVP",
		description: t$1("Returns the standard deviation of a population."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	VAR: {
		name: "VAR",
		description: t$1("Returns the variance of a sample."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	VARP: {
		name: "VARP",
		description: t$1("Returns the variance of a population."),
		parameters: [{
			name: "values",
			description: "Value1, Value2, ...ValueN"
		}]
	...
	PERCENTILE: {
		name: "PERCENTILE",
		description: t$1("Returns the k-th percentile."),
		parameters: [{
			name: "array",
			description: "Array"
		}, {
	...
	QUARTILE: {
		name: "QUARTILE",
		description: t$1("Returns the quartile of a dataset."),
		parameters: [{
			name: "array",
			description: "Array"
		}, {
	...
	RANK: {
		name: "RANK",
		description: t$1("Returns the rank of a number in a list."),
		parameters: [
			{
				name: "value",
				description: "Value"
	...
	IF: {
		name: "IF",
		description: t$1("Returns one value if condition is TRUE, another if FALSE."),
		parameters: [
			{
				name: "condition",
	...
	IFS: {
		name: "IFS",
		description: t$1("Checks multiple conditions and returns corresponding values."),
		parameters: [{
			name: "condition1",
	...
	AND: {
		name: "AND",
		description: t$1("Returns TRUE if all arguments are TRUE."),
		parameters: [{
			name: "conditions",
	...
	OR: {
		name: "OR",
		description: t$1("Returns TRUE if any argument is TRUE."),
		parameters: [{
			name: "conditions",
			description: "Condition1, Condition2, ...ConditionN"
		}]
	...
	XOR: {
		name: "XOR",
		description: t$1("Returns TRUE if odd number of arguments are TRUE."),
		parameters: [{
			name: "conditions",
			description: "Condition1, Condition2, ...ConditionN"
		}]
	...
	NOT: {
		name: "NOT",
		description: t$1("Reverses the logical value."),
		parameters: [{
			name: "condition",
			description: "Condition"
		}]
	...
	TRUE: {
		name: "TRUE",
		description: t$1("Returns the logical value TRUE."),
		parameters: []
	},
	FALSE: {
		name: "FALSE",
		description: t$1("Returns the logical value FALSE."),
		parameters: []
	},
	IFERROR: {
		name: "IFERROR",
		description: t$1("Returns value if no error, otherwise returns alternative."),
		parameters: [{
			name: "value",
			description: "Value"
		}, {
			name: "value_if_error",
			description: "ValueIfError"
		}]
	},
	IFNA: {
		name: "IFNA",
		description: t$1("Returns value if not #N/A error, otherwise returns alternative."),
		parameters: [{
			name: "value",
			description: "Value"
		}, {
			name: "value_if_na",
			description: "ValueIfNA"
		}]
	},
	SWITCH: {
		name: "SWITCH",
		description: t$1("Matches expression against values and returns corresponding result."),
		parameters: [
			{
				name: "expression",
	...
	CONCATENATE: {
		name: "CONCATENATE",
		description: t$1("Combines several text strings into one."),
		parameters: [{
			name: "texts",
			description: "Text1, Text2, ...TextN"
		}]
	},
	INTEGER_TO_AMOUNT: {
		name: "INTEGER_TO_AMOUNT",
		description: t$1("Converts integer amount to decimal amount (e.g., 1234 -> 12.34)."),
		parameters: [{
			name: "integer_amount",
	...
	UPPER: {
		name: "UPPER",
		description: t$1("Converts text to uppercase."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	LOWER: {
		name: "LOWER",
		description: t$1("Converts text to lowercase."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	PROPER: {
		name: "PROPER",
		description: t$1("Capitalizes first letter of each word."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	LEFT: {
		name: "LEFT",
		description: t$1("Returns leftmost characters from text."),
		parameters: [{
			name: "text",
			description: "Text"
		}, {
	...
	RIGHT: {
		name: "RIGHT",
		description: t$1("Returns rightmost characters from text."),
		parameters: [{
			name: "text",
			description: "Text"
		}, {
	...
	MID: {
		name: "MID",
		description: t$1("Returns substring from specified position."),
		parameters: [
			{
				name: "text",
				description: "Text"
	...
	LEN: {
		name: "LEN",
		description: t$1("Returns length of text."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	TRIM: {
		name: "TRIM",
		description: t$1("Removes extra spaces from text."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	SUBSTITUTE: {
		name: "SUBSTITUTE",
		description: t$1("Replaces occurrences of text."),
		parameters: [
			{
				name: "text",
				description: "Text"
	...
	REPLACE: {
		name: "REPLACE",
		description: t$1("Replaces substring at specified position."),
		parameters: [
			{
				name: "text",
				description: "Text"
	...
	FIND: {
		name: "FIND",
		description: t$1("Finds text within text (case-sensitive)."),
		parameters: [{
			name: "find_text",
			description: "Text1"
		}, {
	...
	SEARCH: {
		name: "SEARCH",
		description: t$1("Finds text within text (case-insensitive, supports wildcards)."),
		parameters: [{
			name: "search_text",
	...
	TEXT: {
		name: "TEXT",
		description: t$1("Converts number to text with format."),
		parameters: [{
			name: "value",
			description: "Number"
		}, {
	...
	FIXED: {
		name: "FIXED",
		description: t$1("Formats a number to a fixed amount of decimal places."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	REPT: {
		name: "REPT",
		description: t$1("Repeats text specified number of times."),
		parameters: [{
			name: "text",
			description: "Text"
		}, {
	...
	CHAR: {
		name: "CHAR",
		description: t$1("Converts number to character."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	CODE: {
		name: "CODE",
		description: t$1("Returns numeric code for first character."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	EXACT: {
		name: "EXACT",
		description: t$1("Returns TRUE if texts are exactly the same."),
		parameters: [{
			name: "text1",
			description: "Text"
		}, {
	...
	DATE: {
		name: "DATE",
		description: t$1("Returns date as number of days since null date."),
		parameters: [
			{
				name: "year",
				description: "Year"
	...
	TODAY: {
		name: "TODAY",
		description: t$1("Returns current date."),
		parameters: []
	},
	NOW: {
		name: "NOW",
		description: t$1("Returns current date and time."),
		parameters: []
	},
	YEAR: {
		name: "YEAR",
		description: t$1("Returns the year from a date."),
		parameters: [{
			name: "date",
			description: "Number"
		}]
	...
	MONTH: {
		name: "MONTH",
		description: t$1("Returns the month from a date."),
		parameters: [{
			name: "date",
			description: "Number"
		}]
	...
	DAY: {
		name: "DAY",
		description: t$1("Returns the day from a date."),
		parameters: [{
			name: "date",
			description: "Number"
		}]
	...
	WEEKDAY: {
		name: "WEEKDAY",
		description: t$1("Returns day of week (1-7)."),
		parameters: [{
			name: "date",
			description: "Date"
		}, {
	...
	EDATE: {
		name: "EDATE",
		description: t$1("Returns date shifted by specified months."),
		parameters: [{
			name: "start_date",
	...
	EOMONTH: {
		name: "EOMONTH",
		description: t$1("Returns last day of month after specified months."),
		parameters: [{
			name: "start_date",
			description: "Startdate"
		}, {
	...
	DAYS: {
		name: "DAYS",
		description: t$1("Calculates difference between dates in days."),
		parameters: [{
			name: "end_date",
			description: "Date2"
		}, {
	...
	DATEDIF: {
		name: "DATEDIF",
		description: t$1("Calculates distance between dates."),
		parameters: [
			{
				name: "start_date",
				description: "Date1"
	...
	NETWORKDAYS: {
		name: "NETWORKDAYS",
		description: t$1("Returns number of working days between dates."),
		parameters: [{
			name: "start_date",
			description: "Date1"
		}, {
	...
	WEEKNUM: {
		name: "WEEKNUM",
		description: t$1("Returns week number of year."),
		parameters: [{
			name: "date",
			description: "Date"
		}, {
	...
	VLOOKUP: {
		name: "VLOOKUP",
		description: t$1("Searches vertically in first column and returns value."),
		parameters: [
			{
				name: "lookup_value",
			...
			{
				name: "range_lookup",
				description: "RangeLookup"
			}
		]
	},
	HLOOKUP: {
		name: "HLOOKUP",
		description: t$1("Searches horizontally in first row and returns value."),
		parameters: [
			{
				name: "lookup_value",
				description: "LookupValue"
	...
	INDEX: {
		name: "INDEX",
		description: t$1("Returns value at specified row and column."),
		parameters: [
			{
				name: "array",
				description: "Array"
	...
	MATCH: {
		name: "MATCH",
		description: t$1("Returns position of value in array."),
		parameters: [
			{
				name: "lookup_value",
				description: "LookupValue"
	...
	CHOOSE: {
		name: "CHOOSE",
		description: t$1("Returns value from list based on index."),
		parameters: [
			{
				name: "index",
				description: "Index"
	...
	LOOKUP: {
		name: "LOOKUP",
		description: t$1("Looks up values in a vector or array."),
		parameters: [{
			name: "lookup_value",
			description: "LookupValue"
		}, {
			name: "lookup_vector",
			description: "LookupVector"
		}]
	},
	ISBLANK: {
		name: "ISBLANK",
		description: t$1("Returns TRUE if value is blank."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISERROR: {
		name: "ISERROR",
		description: t$1("Returns TRUE if value is any error."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISNA: {
		name: "ISNA",
		description: t$1("Returns TRUE if value is #N/A error."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISNUMBER: {
		name: "ISNUMBER",
		description: t$1("Returns TRUE if value is a number."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISTEXT: {
		name: "ISTEXT",
		description: t$1("Returns TRUE if value is text."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISLOGICAL: {
		name: "ISLOGICAL",
		description: t$1("Returns TRUE if value is logical (TRUE/FALSE)."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	ISREF: {
		name: "ISREF",
		description: t$1("Returns TRUE if value is a reference."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	PMT: {
		name: "PMT",
		description: t$1("Calculates payment for a loan."),
		parameters: [
			{
				name: "rate",
				description: "Rate"
	...
	FV: {
		name: "FV",
		description: t$1("Calculates future value of investment."),
		parameters: [
			{
				name: "rate",
				description: "Rate"
	...
	PV: {
		name: "PV",
		description: t$1("Calculates present value of investment."),
		parameters: [
			{
				name: "rate",
				description: "Rate"
	...
	NPV: {
		name: "NPV",
		description: t$1("Calculates net present value."),
		parameters: [
			{
				name: "rate",
				description: "Rate"
	...
	IRR: {
		name: "IRR",
		description: t$1("Calculates internal rate of return."),
		parameters: [{
			name: "values",
			description: "Values"
		}]
	...
	RATE: {
		name: "RATE",
		description: t$1("Calculates interest rate per period."),
		parameters: [
			{
				name: "nper",
				description: "Nper"
	...
	QUERY: {
		name: "QUERY",
		description: t$1("Execute a query and return the result."),
		parameters: [{
			name: "queryName",
	...
	QUERY_COUNT: {
		name: "QUERY_COUNT",
		description: t$1("Execute a query and return the number of matching rows."),
		parameters: [{
			name: "queryName",
			description: "Name of the query to execute"
		}]
...
//#endregion
//#region src/components/formula/transactionModeFunctions.ts
var transactionModeFunctions = {
	CONCATENATE: {
		name: "CONCATENATE",
		description: t$1("Combines several text strings into one."),
		parameters: [{
	...
	BALANCE_OF: {
		name: "BALANCE_OF",
		description: t$1("Running balance for another account (cents) at this transaction, same cutoff as balance. Use a quoted account id for a deterministic match, or a quoted account name. Use the balance variable instead for the current account."),
		parameters: [{
			name: "account_id_or_name",
			description: t$1("Quoted account id or exact account name")
		}]
	},
	MID: {
		name: "MID",
	...
	CLEAN: {
		name: "CLEAN",
		description: t$1("Removes non-printable characters from text."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	SPLIT: {
		name: "SPLIT",
		description: t$1("Splits text by space and returns part at index."),
		parameters: [{
			name: "text",
			description: "Text"
		}, {
	...
	SUM: {
		name: "SUM",
		description: t$1("Returns the sum of all numbers."),
		parameters: [{
			name: "numbers",
			description: "Number1, Number2, ...NumberN"
		}]
	...
	INT: {
		name: "INT",
		description: t$1("Rounds down to nearest integer."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	TRUNC: {
		name: "TRUNC",
		description: t$1("Truncates number to specified decimals."),
		parameters: [{
			name: "number",
			description: "Number"
		}, {
	...
	SIGN: {
		name: "SIGN",
		description: t$1("Returns -1 for negative, 0 for zero, 1 for positive."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	DATEVALUE: {
		name: "DATEVALUE",
		description: t$1("Parses a date string and returns it as a number."),
		parameters: [{
			name: "date_string",
	...
	ISOWEEKNUM: {
		name: "ISOWEEKNUM",
		description: t$1("Returns ISO week number."),
		parameters: [{
			name: "date",
			description: "Date"
		}]
	...
	ISEVEN: {
		name: "ISEVEN",
		description: t$1("Returns TRUE if number is even."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	ISODD: {
		name: "ISODD",
		description: t$1("Returns TRUE if number is odd."),
		parameters: [{
			name: "number",
			description: "Number"
		}]
	...
	VALUE: {
		name: "VALUE",
		description: t$1("Converts text to a number."),
		parameters: [{
			name: "text",
			description: "Text"
		}]
	...
	T: {
		name: "T",
		description: t$1("Returns text if value is text, empty string otherwise."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
	...
	N: {
		name: "N",
		description: t$1("Converts value to a number."),
		parameters: [{
			name: "value",
			description: "Value"
		}]
...
//#endregion
//#region src/components/formula/codeMirror-excelLanguage.tsx
function FunctionTooltip({ name, description, parameters }) {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trans, { children: "Parameters:" })
				}), parameters.map((p, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
...
function FieldTooltip({ label, info }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		style: { maxWidth: "400px" },
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			style: {
			...
			children: label
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: info })]
	});
}
...
var excelFormulaParser = {
	...
	token(stream, state) {
		...
		if (stream.match(/^[A-Z_][A-Z0-9_]*/)) {
			...
			if (stream.peek() === "(") {
				stream.pos = pos;
				if (MATH_FUNCTIONS.has(word)) return "keyword";
				if (LOGICAL_FUNCTIONS.has(word)) return "className";
				if (TEXT_FUNCTIONS.has(word)) return "namespace";
				if (DATE_FUNCTIONS.has(word)) return "typeName";
				if (QUERY_FUNCTIONS.has(word)) return "propertyName";
				return "function";
			}
			stream.pos = pos;
			return "variableName";
...
var transactionFields = [
	{
		...
		boost: 5,
		info: t$1("Transaction amount in cents. Use for calculations and comparisons.\n\nExample: `=amount / 100` to get dollar value")
	},
	{
		label: "date",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Transaction date in YYYY-MM-DD format. Use with date functions.\n\nExample: `=TEXT(date, \"MMMM\")` to get month name")
	},
	{
		label: "notes",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Transaction notes/memo text. Use for string operations.\n\nExample: `=UPPER(notes)` to convert to uppercase")
	},
	{
		...
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Original imported payee name from bank import. Contains the raw text before matching.\n\nExample: `=LEFT(imported_payee, 10)` to get first 10 characters")
	},
	{
		label: "payee",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Payee ID (string). The ID of the payee.\n\nExample: `=CONCATENATE(\"Payment to \", payee)`")
	},
	{
		label: "payee_name",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Payee name (string). The human-readable name of the payee.\n\nExample: `=UPPER(payee_name)` or `=CONCATENATE(\"Payment to \", payee_name)`")
	},
	{
		label: "account",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Account ID (string). The ID of the account.\n\nExample: `=CONCATENATE(\"Paid from \", account)`")
	},
	{
		label: "account_name",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Account name (string). The human-readable name of the account.\n\nExample: `=CONCATENATE(\"Paid from \", account_name)`")
	},
	{
		label: "category",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Category ID (string). The ID of the category.\n\nExample: `=IF(category=\"Groceries\", \"Food\", \"Other\")`")
	},
	{
		label: "category_name",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Category Name (string). The human-readable name of the category.\n\nExample: `=IF(category_name=\"Groceries\", \"Food\", \"Other\")`")
	},
	{
		label: "cleared",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Boolean cleared status. TRUE if transaction is cleared, FALSE otherwise.\n\nExample: `=IF(cleared, \"Cleared\", \"Pending\")`")
	},
	{
		label: "reconciled",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Boolean reconciled status. TRUE if transaction is reconciled, FALSE otherwise.")
	},
	{
		label: "balance",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("Account balance as of the date of the transaction, excluding the transaction amount. Use for calculations and comparisons.\n\nExample: `=IF(balance < 0, \"Negative Balance\", \"Positive Balance\")`")
	},
	{
		label: "parent_amount",
		type: "variable",
		section: "💰 Transaction Fields",
		boost: 5,
		info: t$1("The amount of the parent transaction in cents in split transactions.\n\nExample: `=(parent_amount / 100) * .05`")
	}
];
function getFunctionCompletions(mode) {
	const functions = mode === "query" ? queryModeFunctions : transactionModeFunctions;
	const createCompletion = (name, func, section) => ({
		label: name,
		type: "function",
		section,
		info: [
			func.description,
			"",
			`${t$1("Parameters:")} ${func.parameters.map((p) => p.name).join(", ")}`,
			"",
	...
	const completions = [];
	for (const [name, func] of Object.entries(functions)) if (MATH_FUNCTIONS.has(name)) completions.push(createCompletion(name, func, "📊 Math Functions"));
	else if (LOGICAL_FUNCTIONS.has(name)) completions.push(createCompletion(name, func, "🔀 Logical Functions"));
	else if (TEXT_FUNCTIONS.has(name)) completions.push(createCompletion(name, func, "📝 Text Functions"));
	else if (DATE_FUNCTIONS.has(name)) completions.push(createCompletion(name, func, "📅 Date Functions"));
	else if (QUERY_FUNCTIONS.has(name)) completions.push(createCompletion(name, func, "🔍 Query Functions"));
	else completions.push(createCompletion(name, func, "⚙️ Other Functions"));
	return completions;
}
function excelFormulaAutocomplete(mode, queries, variables) {
	const functionCompletions = getFunctionCompletions(mode);
	const queryCompletions = queries ? Object.keys(queries).flatMap((queryName) => [
		{
			label: `QUERY("${queryName}")`,
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Execute the {{queryName}} query and return the result.", { queryName }),
		...
		},
		{
			...
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Execute the {{queryName}} query and return the number of matching rows.", { queryName }),
		...
		},
		{
			...
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Sum of budgeted amounts with extracted parameters from {{queryName}}.", { queryName }),
		...
		},
		{
			...
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Sum of spending with extracted parameters from {{queryName}}.", { queryName }),
		...
		},
		{
			...
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Opening balance with extracted parameters from {{queryName}}.", { queryName }),
		...
		},
		{
			...
			type: "function",
			section: "🔍 Query Functions",
			info: t$1("Closing balance with extracted parameters from {{queryName}}.", { queryName }),
	...
	const variableCompletions = variables ? Object.entries(variables).map(([varName, value]) => ({
		label: varName,
		type: "variable",
		section: "🔢 Variables",
		info: t$1("Variable with value: {{value}}", { value: String(value) }),
		boost: 20
	})) : [];
	return autocompletion({ override: [(context) => {
		...
		if (mode === "transaction") suggestions.push(...transactionFields);
		const sortedSuggestions = suggestions.sort((a, b) => {
			...
			const sectionA = typeof a.section === "string" ? a.section : a.section?.name || "";
			const sectionB = typeof b.section === "string" ? b.section : b.section?.name || "";
...
function excelFormulaHover(mode) {
	const functions = mode === "query" ? queryModeFunctions : transactionModeFunctions;
	return hoverTooltip((view, pos) => {
		...
		if (funcDef) return {
			...
			create() {
				...
				root.render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FunctionTooltip, {
					name: upper,
		...
		if (mode === "transaction") {
			...
			if (field) return {
				...
				create() {
					...
					root.render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FieldTooltip, {
						label: field.label,
						info: infoText
					}));
					return {
...
var excelFormulaLanguage = StreamLanguage.define(excelFormulaParser);
var tooltipZIndexTheme = EditorView.baseTheme({
	".cm-tooltip": { zIndex: "4000 !important" },
	".cm-tooltip-autocomplete": { zIndex: "4000 !important" }
});
var tooltipPortalConfig = typeof document === "undefined" ? tooltips({ position: "fixed" }) : tooltips({
	position: "fixed",
	parent: document.body
});
var autocompletePopoverTheme = EditorView.baseTheme({
	...
	".cm-tooltip.cm-tooltip-autocomplete > ul": {
		...
		minWidth: "280px",
		listStyle: "none",
		backgroundColor: theme$1.menuBackground,
		color: theme$1.menuItemText,
		borderRadius: "6px",
		overflow: "hidden",
		overflowY: "auto"
	},
	...
	".cm-tooltip.cm-tooltip-autocomplete .cm-completionIcon::before": { display: "none !important" },
	".cm-tooltip.cm-tooltip-autocomplete li.cm-completionSection": {
		...
		color: theme$1.menuItemTextHeader,
		opacity: .9,
		marginTop: "8px"
	},
	".cm-tooltip.cm-tooltip-autocomplete li.cm-completionSection:first-child": { marginTop: 0 },
	...
	".cm-tooltip.cm-tooltip-autocomplete li.cm-completionItem[aria-selected]": {
		backgroundColor: theme$1.menuItemBackgroundHover,
		color: theme$1.menuItemTextHover
	},
	...
	".cm-tooltip.cm-tooltip-autocomplete .cm-completionDetail": {
		fontSize: "12px",
		color: theme$1.menuKeybindingText,
		marginLeft: "auto",
		whiteSpace: "nowrap"
	},
	".cm-tooltip.cm-tooltip-autocomplete li.cm-completionItem[aria-selected] .cm-completionDetail": { color: theme$1.menuItemTextHover },
	".cm-tooltip.cm-tooltip-autocomplete .cm-completionInfo": {
		backgroundColor: theme$1.menuBackground,
		color: theme$1.pageText,
		padding: "12px 14px",
		maxWidth: "360px",
		whiteSpace: "pre-wrap",
		fontSize: "12px",
...
var functionCategoryTheme = EditorView.baseTheme({
	"&light .cm-keyword": { color: "#0066CC" },
	"&light .cm-className": { color: "#267F99" },
	"&light .cm-namespace": { color: "#AF00DB" },
	"&light .cm-typeName": { color: "#098658" },
	"&light .cm-propertyName": { color: "#D73A49" }
});
var excelFormulaHighlighting = syntaxHighlighting(HighlightStyle.define([
	{
...
var functionCategoryThemeDark = EditorView.baseTheme({
	"&dark .cm-keyword": { color: "#4FC1FF" },
	"&dark .cm-className": { color: "#4EC9B0" },
	"&dark .cm-namespace": { color: "#C586C0" },
	"&dark .cm-typeName": { color: "#B5CEA8" },
	"&dark .cm-propertyName": { color: "#F97583" }
});
var excelFormulaDarkHighlighting = syntaxHighlighting(HighlightStyle.define([
	{
		tag: tags.function(tags.variableName),
		color: "#DCDCAA"
	},
...
function excelFormulaExtension(mode, queries, isDark, variables) {
	return [
...
//#endregion
//#region src/components/formula/FormulaEditor.tsx
function FormulaEditor({ value, onChange, mode, height = "100%", disabled = false, queries, variables, singleLine = false, showLineNumbers = true }) {
	const [activeTheme] = useTheme();
	const isDarkTheme = (0, import_react.useMemo)(() => {
		if (activeTheme === "dark" || activeTheme === "midnight") return true;
		if (activeTheme === "auto") return window.matchMedia("(prefers-color-scheme: dark)").matches;
		return false;
	}, [activeTheme]);
	const extensions = (0, import_react.useMemo)(() => [
		...singleLine ? [EditorState.transactionFilter.of((tr) => tr.newDoc.lines > 1 ? [tr, {
			changes: {
				from: 0,
				to: tr.newDoc.length,
				insert: tr.newDoc.sliceString(0, void 0, " ")
			},
			sequential: true
		}] : [tr])] : [],
...
//# sourceMappingURL=FormulaEditor.BCcTZYRx.chunk.js.map